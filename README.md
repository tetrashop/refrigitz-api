...
#### ب) روش لبه‌محور (Edge Magnitude) – راه‌حل نهایی
...
- **سطوح کاملاً صاف** (با فیلتر گوسی قوی).
- **جزئیات ظریف** (متن، خطوط، بافت) به‌خوبی نمایان می‌شوند.
...
| **سطوح خشن و تیز** (رشته‌کوه) | کاهش `sigma` فیلتر گوسی، افزایش تراکم نقاط | **فیلتر گوسی قوی** (`sigma=4`) روی نقشهٔ لبه |
...
│   ├── obj_generator.py      # تولید مدل OBJ (روش لبه‌محور)
...
├── vercel.json               # (اختیاری) تنظیمات زمان‌بندی
...
از تمام ابزارهای متن‌باز استفاده‌شده (Flask، Pillow، SciPy، NumPy) و همچنین پلتفرم Vercel برای میزبانی رایگان تشکر می‌کنیم.
